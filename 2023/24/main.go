...
// Source: https://github.com/rumkugel13/AdventOfCode2023/blob/main/day24.go
...
func main() {
	...
	fmt.Println("Part 1: How many of these intersections occur within the test area?")
	fmt.Println(result)
	...
	if len(maybeX) == len(maybeY) && len(maybeY) == len(maybeZ) && len(maybeZ) == 1 {
		...
		result2 = int(xPos+yPos+zPos) + 1
	}

	fmt.Println()
	fmt.Println("Part 2: What do you get if you add up the X, Y, and Z coordinates of that initial position?")
	fmt.Println(result2)
}
