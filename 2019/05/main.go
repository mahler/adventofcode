...
func main() {
	dat, err := ioutil.ReadFile("puzzle.txt")
	if err != nil {
		panic(err)
	}
	...
	r1 := make([]int, len(r))
	copy(r1, r)
	result := doRun(r1, []int{1})
	fmt.Println()
	fmt.Println("Day 05: Sunny with a Chance of Asteroids")
	fmt.Println(result)

	r2 := make([]int, len(r))
	copy(r2, r)
	result = doRun(r2, []int{5})
	fmt.Println()
	fmt.Println("Part 2")
	fmt.Println(result)
}

func doRun(r []int, input []int) []int {
	outputs := []int{}
	ip := 0
	for {
		op := LoadOp(r[ip])
